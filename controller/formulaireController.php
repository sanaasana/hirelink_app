<?php
require_once '../model/formulaireModel.php'; // Inclusion du modèle

class FormulaireController {
    public function traiterFormulaire() {
        // Récupérer le dernier ID
        $formModel = new FormulairetModel();     

        // Récupérer les données du formulaire
        $id_ct = $contactModel->dernierIdContact();
        $email = $_POST['email'];
        ...
        $id_f = $contactModel->dernierIdFormation();
        $nom = $_POST['nom'];
        ...
        $id_f = $contactModel->dernierIdProfil();
        $nom = $_POST['profil'];
        $id_cv=1;

        // Récupérer les données du formulaire de experience
        $id_exp = $contactModel->dernierIdExp();
        ...
        $id_comp = $contactModel->dernierIdCompetences();
        $Competence = $_POST['Competence'];        
        $niveau = $_POST['niveau'];
        $id_cv=1;

        // Récupérer les données du formulaire de langues
        $id_langue = $contactModel->dernierIdLangues();
        $nom = $_POST['Langue'];        
        $niveau = $_POST['niveau'];
        $id_cv=1;
